...
class NaviControl():
  def __init__(self):
    ...
    self.map_spdlimit_offset = int(Params().get("KisaSpeedLimitOffset", encoding="utf8"))
    self.map_spdlimit_offset_option = int(Params().get("KisaSpeedLimitOffsetOption", encoding="utf8"))
    ...
    self.decel_on_speedbump = Params().get_bool("KISASpeedBump")
    self.navi_sel = int(Params().get("KISANaviSelect", encoding="utf8"))
  ...
  def auto_speed_control(self, CS, navi_speed):
    ...
    cut_in_model = True if self.lead_1.status and (self.lead_0.dRel - self.lead_1.dRel) > 3.0 and ((0 < self.lead_0.dRel < 96) or (0 < self.lead_1.dRel < 96)) else False
    dist_sel = self.lead_1.dRel if 0 < self.lead_1.dRel < 96 else self.lead_0.dRel if 0 < self.lead_0.dRel < 96 else CS.lead_distance
    cut_in_ed_rd_diff = True if 0 < CS.lead_distance <= 102 and (CS.lead_distance - dist_sel) > interp(CS.lead_distance, [10, 50], [3.0, 9.0]) else False
